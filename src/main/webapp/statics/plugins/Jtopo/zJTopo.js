...
//清楚所有元素
JTopo.Scene.prototype.clear = function () {
...
JTopo.Scene.prototype.zFocusEle = function (ele, params, callback) {
    ...
    $(this).animate({
        ...
        translateY:-ele.y+params.y+params.translateY
...
JTopo.Node.prototype.zGetLinks = function () {
    var links = [];
    if (this.outLinks) {
        links  = links.concat(this.outLinks);
    }
    if (this.inLinks) {
        links = links.concat(this.inLinks);
    }
    return links;
}

JTopo.Node.prototype.zGetLinkEles = function () {
    var links = this.zGetLinks();
    var eles = [];
    for (var i = 0; i < links.length; i++) {
        var link = links[i];
        var linkEle = link.nodeZ==this?link.nodeA:link.nodeZ;
        if (linkEle.elementType == "node" && linkEle.entity && linkEle.entity.deviceId) {
            eles.push(linkEle);
        }
    }
    return eles;
...
JTopo.Node.prototype.zGetLinkNodes = function () {
    var links = this.zGetLinks();
    var eles = [];
    for (var i = 0; i < links.length; i++) {
        var link = links[i];
        var linkEle = link.nodeZ == this ? link.nodeA : link.nodeZ;
        if (linkEle.elementType == "node") {
            eles.push(linkEle);
        }
    }
    return eles;
...
JTopo.Node.prototype.zGetAbsolutePosition = function () {

    var c = this.scene.stage.canvas

    var border_top_width = jQuery(c).css('border-top-width');
    var border_left_width = jQuery(c).css('border-left-width');
    border_top_width = isNaN(parseInt(border_top_width)) ? 0 : border_top_width;
    border_left_width = isNaN(parseInt(border_left_width)) ? 0 : border_left_width;
    var overlay_left_pos = jQuery(c).offset().left + parseInt(border_left_width);
    var overlay_top_pos = jQuery(c).offset().top + parseInt(border_top_width);

    return {
        left: overlay_left_pos + this.x + this.scene.translateX*this.scene.scaleX+(this.x-c.width/2)*(this.scene.scaleX-1),
        top: overlay_top_pos + this.y + this.scene.translateY*this.scene.scaleY + (this.y - c.height / 2) * (this.scene.scaleY - 1),
    };



    //params.x = (ev.clientX - $("#canvas").offset().left - canvas.width() / 2) / stage.curScene.scaleX + canvas.width() / 2 - params.width / 2 - stage.curScene.translateX;
    //params.y = (ev.clientY - $("#canvas").offset().top - canvas.height() / 2) / stage.curScene.scaleY + canvas.height() / 2 - params.height / 2 - stage.curScene.translateY;
}
...
JTopo.Node.prototype.zOpen = function ()
{
    for (var i = 0; i < stage.scenes.length; i++)
    {
        if (stage.scenes[i].entity && stage.scenes[i].entity.pid == this.entity.id)
        {
            ...
            console.log(stage.curScene.entity);
            try{
                setCurUrlQuery("topologyId", stage.curScene.entity.id);
            } catch (e) {
                console.log(e);
            }
...
JTopo.Stage.prototype.downImgag = function (filename) {
    ...
    var dataUrl = this.eagleEye.getImage();
    $.messager.progress({title:"生成中",msg:"生成图片中"});
    $.post("/service/ResourceService.ashx?action=downloadImageByBase64", { base64: dataUrl }, function (result) {

        if (result.success === false) {
            zAlert(result.msg, "下载图片出错", "error");
            return;
        }

        var save_link = document.createElementNS("http://www.w3.org/1999/xhtml", "a");
        save_link.href = result.path;
        save_link.src = result.path;
